...
blib::math::AABB::AABB(const glm::vec3 &min, const glm::vec3 &max) : min(bounds[0]), max(bounds[1])
{
	bounds[0] = min;
	bounds[1] = max;
}
...
*/
bool blib::math::AABB::hasRayCollision(const Ray& r, float minDistance, float maxDistance) const
{
